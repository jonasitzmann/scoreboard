...
ScoreboardController::ScoreboardController()
  ...
  configLoader = make_shared<WhiteZeroLoader>();
...
void ScoreboardController::collectInputCommands() {
  inputCommands.clear();
  for_each(begin(inputDevices), end(inputDevices),
           [&](shared_ptr<InputDevice> device) {
             auto newInputs = device->getInput();
             inputCommands.insert(end(inputCommands), begin(newInputs),
                                  end(newInputs));
           });
}
...
bool ScoreboardController::executeInputCommands() {
  ...
  for (auto outputItr = outputDevices.begin(); outputItr != outputDevices.end();
       ++outputItr) {
...
void ScoreboardController::addOutputDevice(
